...
bool tud_usbtmc_msg_data_cb(void *data, size_t len, bool transfer_complete)
{
    ...
    if ( transfer_complete && (len >= 4) && (!strncmp("*idn?", (const char*)data, 4) || !strncmp("*IDN?", (const char*)data, 4)) )
    {
        idnQuery = 1;
    }

    if ( transfer_complete && (!strncmp("delay ", (const char*)data, 5) || !strncmp("DELAY ", (const char*)data, 5)) )
    {
        queryState = 0;
...
bool tud_usbtmc_msgBulkIn_request_cb(usbtmc_msg_request_dev_dep_in const * request)
{
    ...
    else
    {
        ...
        tud_usbtmc_transmit_dev_msg_data(&buffer[buffer_tx_ix], txlen, (buffer_tx_ix+txlen) == buffer_len, false);
        buffer_tx_ix += txlen;
    }
...
void usbtmc_app_task_iter(void) 
{
    ...
    case 0:
        break;
    case 1:
        queryDelayStart = (uint32_t)systick_ms();
        queryState = 2;
        break;
    case 2:
        if( ((uint32_t)systick_ms() - queryDelayStart) > resp_delay)
        {
            queryDelayStart = (uint32_t)systick_ms();
            queryState=3;
            status |= 0x10u; // MAV
